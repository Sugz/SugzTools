...
namespace CodeDoc.Src
{
    public static class CDParser
    {
        ...
        /// <summary>
        /// Return a formated run to be use as title
        /// </summary>
        ...
        /// <returns></returns>
        public static bool GetTitle(StringCollection strs, int index, Paragraph paragraph)
        {
            bool isTitle = false;
            if (strs[index].EndsWith(":"))
            {
                isTitle = true;
                // Skip empty titles
                if (!(index + 1 == strs.Count) && !(strs[index + 1].EndsWith(":")))
                {
