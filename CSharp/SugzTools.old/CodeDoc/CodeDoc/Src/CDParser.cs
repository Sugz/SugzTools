    ...
    public static class CDParser
    {
        ...
        public static bool IsList(StringCollection strs, int index, FlowDocument document)
        {
            ...
            if (strs[index].StartsWith("-"))
            {
                ...
                List list = new List(new ListItem(GetHyperlinkTypesAndNormalText(str)));
        ...
        public static Paragraph GetHyperlinkTypesAndNormalText(string line)
        {
            Paragraph paragraph = new Paragraph();
            int count = 0;
            foreach (string s in line.SplitAndKeep(CDConstant.Delimiters.StringArrayToCharArray()))
            {
                // Get hyperlink
